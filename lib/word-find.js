...
WordPuzzle.prototype.fillPuzzle = function (words, height, width) {
  ...
  for (i =0; i < words.length; i++) {
    ...
    if (locations.length === 0) {
      ...
      // pop the remaining words as they dont fit
      ...
      solution.push({word: currentWord, location: location});
  ...
  return {puzzle: puzzle, solution: solution, words: words};
};
...
WordPuzzle.prototype.newPuzzle = function newPuzzle(height, width) {
  ...
  var result = this.fillPuzzle(words, this.height, this.width);
  this.fillBlanks(result.puzzle);
  console.log('finally puzzle is created !!!');
  // return this.prettyPrint(puzzle);
  return {result: result, wordsList: words};
}
